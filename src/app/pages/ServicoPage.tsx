...
export default function ServicoPage() {
  ...
  /* Serviços sem conteúdo rico (8 dos 16) renderizam um detalhe enxuto:
     ...
     sem coluna de imagem. */
  const hasImage = Boolean(svc.image);
  const paragraphs = svc.fullDesc ? svc.fullDesc.split("\n\n") : [svc.shortDesc];
    ...
    <>
      ...
      <section style={{ backgroundColor: "#fff", paddingTop: 0, paddingBottom: 100 }}>
        <div style={{ maxWidth: 1440, margin: "0 auto", padding: "0 30px" }}>
          <AnimateOnScroll>
            <div className={`grid grid-cols-1 gap-16 lg:gap-24 items-center ${hasImage ? "lg:grid-cols-[1fr_1.1fr]" : ""}`}>
      ...
      {/* ═══════════════════════════════════════════════════
          3) O QUE INCLUI — numbered list (oculto quando sem itens)
         ═══════════════════════════════════════════════════ */}
