...
**Resultado atual:** G3 PENDENTE — IMPLEMENTAÇÃO LOCAL PRONTA PARA VALIDAÇÃO<br>
...
| Execução pgTAP integral     | pendente            | host local sem Docker Desktop ou Podman; será executada no job `database` da CI |
...
EV2.4 não está liberada. Para avançar, ainda são necessários o resultado verde da CI de banco e autorização específica do canary/piloto EV2.3. A autorização anterior da EV2.2 não foi reutilizada nem ampliada.
