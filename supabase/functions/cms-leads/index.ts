...
Deno.serve(async(req)=>{
  ...
  if(error){const marker=error.message.match(/CMS_[A-Z0-9_]+/)?.[0],forbidden=error.message.includes("FORBIDDEN")||error.message.includes("FEATURE_DISABLED")||error.code==="42501",notFound=error.message.includes("NOT_FOUND")||error.code==="PT404",conflict=error.message.includes("CONFLICT")||error.message.includes("NOT_RETRYABLE")||error.code==="PT409",rateLimited=marker==="CMS_RATE_LIMIT_EXCEEDED"||error.code==="PT429",invalid=error.message.includes("INVALID")||error.code==="23514"||error.code==="22023";return json(req,{error:forbidden?"Permissão insuficiente ou recurso não habilitado.":notFound?"Registro não encontrado.":conflict?"A entrega não pode ser reprocessada no estado atual.":rateLimited?"Muitas operações. Aguarde.":invalid?"Dados ou transição inválidos.":"Falha na operação.",code:marker,correlationId,preserved:input.action==="retry_delivery"},forbidden?403:notFound?404:conflict?409:rateLimited?429:invalid?422:500);}
