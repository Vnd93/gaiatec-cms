...
export function redactAiText(input: string, maxLength = 6000) {
  ...
  return {
    ...
    blocked,
    risk: blocked ? ("blocked" as const) : categories.size ? ("redacted" as const) : ("safe" as const),
  };
}
...
const INJECTION_SIGNALS = [
  ...
  },
  {
    ...
    pattern: /\b(?:bypass|contorne|ignore)\b.{0,60}\b(?:allowlist|permissão|permission|mfa|política|policy)\b/i,
  },
  {
...
export function detectAiPromptInjection(input: string) {
  const normalized = String(input ?? "").normalize("NFKC").slice(0, 6000);
