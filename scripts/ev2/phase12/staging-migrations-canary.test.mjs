...
test("database preflights require every 0082-0088 contract, exact ACL and semantic guards", async () => {
  ...
  for (const verifier of [canary, ...verifiers]) {
    ...
    assert.match(verifier, /system_snapshot_open_critical_scale_0099_semantics_exact/);
    assert.match(verifier, /systemSnapshotOpenCriticalScaleSemanticSql/);
    assert.match(verifier, /system_snapshot_lead_read_scale_0100_semantics_exact/);
    assert.match(verifier, /systemSnapshotLeadReadScaleSemanticSql/);
    assert.match(verifier, /release_stability_followup_0101_semantics_exact/);
    ...
    assert.match(verifier, /lead_retry_subphase_timing_0103_semantics_exact/);
    assert.match(verifier, /leadRetrySubphaseTimingSemanticSql/);
    ...
    assert.match(verifier, /blog_taxonomy_terminal_cleanup_0106_functions_locked/);
    assert.match(verifier, /blog_taxonomy_terminal_cleanup_0106_semantics_exact/);
    assert.match(verifier, /blogTaxonomyTerminalCleanupSemanticSql/);
    assert.match(verifier, /CMS_BLOG_TAXONOMY_TERMINAL_CLEANUP_0106_OWNER_ONLY_FUNCTIONS/);
  }
...
test("canary proves refresh-resistant session revocation without banning Auth", async () => {
  ...
  assert.match(source, /"0091"/);
  assert.match(source, /"0092"/);
