...
import { verifyStagingReleasePackage } from "./staging-release-package-lib.mjs";

const ALLOWED_ARGUMENTS = new Set(["candidate", "output", "package", "profile", "run-attempt", "run-id"]);
...
function parseArguments(argv) {
  ...
  for (const required of ["package", "candidate", "profile", "run-id", "run-attempt"])
    if (!options[required]) throw new Error(`--${required} is required`);
  return options;
}

const options = parseArguments(process.argv.slice(2));
const result = await verifyStagingReleasePackage({
  packageDirectory: options.package,
...
process.stdout.write(
  `${JSON.stringify({ event: "staging.release_package.verified", ...result, manifest: undefined })}\n`,
);
