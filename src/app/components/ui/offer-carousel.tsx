...
const OfferCard = React.forwardRef<HTMLAnchorElement, OfferCardProps>(({ offer }, ref) => (
  <motion.a
    ref={ref}
    href={offer.href}
    className="relative flex-shrink-0 w-[224px] h-[315px] rounded-xl overflow-hidden group snap-start bg-white border border-gray-100 shadow-sm"
    whileHover={{ y: -7 }}
  ...
  >
    ...
    />
    {/* Card Content */}
    <div className="absolute bottom-0 left-0 right-0 h-1/2 bg-white p-4 flex flex-col justify-between border-t border-gray-100">
      ...
      <div className="flex items-center justify-between pt-3 border-t border-gray-100">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            {offer.promoCode && (
              <p className="text-[10px] text-gray-500">{offer.promoCode}</p>
            )}
          </div>
        </div>
...
const OfferCarousel = React.forwardRef<HTMLDivElement, OfferCarouselProps>(
  ({ offers, className, ...props }, ref) => {
    ...
    return (
      <div ref={ref} className={cn("relative w-full px-12 hover-container", className)} {...props}>
        {/* Left Scroll Button */}
        <button
          onClick={() => scroll("left")}
        ...
        <div
          ...
          className="flex space-x-8 overflow-x-auto py-10 scrollbar-hide snap-x snap-mandatory px-6"
